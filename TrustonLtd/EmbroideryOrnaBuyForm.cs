...
{
    public partial class EmbroideryOrnaBuyForm : Form
    {
        TrustonEntities db = new TrustonEntities();
        public EmbroideryOrnaBuyForm()
        {
            InitializeComponent();
            LoadData();
        }
        ...
        private void pieceTextBox_Leave(object sender, EventArgs e)
        {
            ...
            {
                errorProvider3.SetError(this.pieceTextBox, "Please enter piece of number!");
                pieceTextBox.Focus();
            }
            else
            {
        ...
        private void productNameComboBox_Leave(object sender, EventArgs e)
        {
            ...
            {
                errorProvider2.SetError(this.productNameComboBox, "Please select product name");
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            try
            {
                BuyEmbroideryOrnaOrThreePiece buyEmbroideryOrna = new BuyEmbroideryOrnaOrThreePiece();
                buyEmbroideryOrna.DateTime = System.DateTime.Now;
                var productId = db.Products.FirstOrDefault(model => (string)productNameComboBox.SelectedItem == model.ProductName);
                if (productId != null) buyEmbroideryOrna.ProductId = productId.ProductId;
                buyEmbroideryOrna.Piece = Convert.ToInt32(pieceTextBox.Text);
                buyEmbroideryOrna.TotalAmount = Convert.ToInt32(totalAmountTextBox.Text);

                db.BuyEmbroideryOrnaOrThreePieces.Add(buyEmbroideryOrna);
